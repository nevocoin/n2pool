	...
	for (int i = 0; i < 2; ++i) {
		...
		do {
			...
			if (name == "generate_key_derivation") {
				...
				uint8_t view_tag;
				ASSERT_EQ(n2pool::generate_key_derivation(key1, key2, 0, derivation, view_tag), result);
				if (result) {
				...
				uint8_t view_tag;
				n2pool::derive_view_tag(derivation, output_index, view_tag);
				...
				hash pub, sec;
				n2pool::get_tx_keys(pub, sec, wallet_spend_key, monero_block_id);
