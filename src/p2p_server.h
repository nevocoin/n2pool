...
static constexpr int DEFAULT_P2P_PORT = 41518;
static constexpr int DEFAULT_P2P_PORT_MINI = 41519;
...
public:
	...
	explicit P2PServer(n2pool *pool);
	~P2PServer();
	...
	struct P2PClient : public Client
	{
		...
		void handle_incoming_block(n2pool* pool, PoolBlock& block, std::vector<hash>& missing_blocks, bool& result);
		void post_handle_incoming_block(n2pool* pool, const PoolBlock& block, const uint32_t reset_counter, bool is_v6, const raw_ip& addr, std::vector<hash>& missing_blocks, const bool result);
