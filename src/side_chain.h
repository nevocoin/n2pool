...
{
public:
	SideChain(n2pool* pool, NetworkType type, const char* pool_name = nullptr);
	~SideChain();
...
namespace robin_hood {
	...
	template<>
	struct hash<n2pool::MinerShare>
	{
		FORCEINLINE size_t operator()(const n2pool::MinerShare& value) const noexcept
		{
			return hash_bytes(value.m_wallet->spend_public_key().h, n2pool::HASH_SIZE);
		}
	};
