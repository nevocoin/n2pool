...
#define ROBIN_HOOD_MALLOC(size) n2pool::malloc_hook(size)
#define ROBIN_HOOD_CALLOC(count, size) n2pool::calloc_hook((count), (size))
#define ROBIN_HOOD_FREE(ptr) n2pool::free_hook(ptr)
...
template<>
struct hash<n2pool::hash>
{
	FORCEINLINE size_t operator()(const n2pool::hash& value) const noexcept
	{
		return hash_bytes(value.h, n2pool::HASH_SIZE);
	}
};
...
template<>
struct hash<n2pool::raw_ip>
{
	FORCEINLINE size_t operator()(const n2pool::raw_ip& value) const noexcept
	{
