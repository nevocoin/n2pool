...
class n2pool : public MinerCallbackHandler, public nocopy_nomove
{
public:
	n2pool(int argc, char* argv[]);
	~n2pool();
...
private:
	n2pool(const n2pool&) = delete;
	n2pool(n2pool&&) = delete;

	const Params::Host& switch_host();

	static void on_submit_block(uv_async_t* async) { reinterpret_cast<n2pool*>(async->data)->submit_block(); }
	static void on_update_block_template(uv_async_t* async) { reinterpret_cast<n2pool*>(async->data)->update_block_template(); }
	static void on_stop(uv_async_t*);
	static void on_reconnect_to_host(uv_async_t* async) { reinterpret_cast<n2pool*>(async->data)->reconnect_to_host(); }
