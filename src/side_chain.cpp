...
static constexpr size_t UNCLE_BLOCK_DEPTH = 2;
...
static constexpr uint8_t default_consensus_id[HASH_SIZE] = { 1,28,159,227,247,86,146,183,82,135,38,200,245,176,227,41,210,188,12,185,192,163,123,64,87,222,137,224,188,241,24,190 };
static constexpr uint8_t mini_consensus_id[HASH_SIZE] = { 98,143,138,205,245,43,132,189,104,103,145,201,115,188,94,1,210,220,137,244,121,93,181,50,136,192,26,13,193,192,167,255 };

NetworkType SideChain::s_networkType = NetworkType::Invalid;

SideChain::SideChain(n2pool* pool, NetworkType type, const char* pool_name)
	: m_pool(pool)
	, m_chainTip{ nullptr }
...
#endif
{
	...
	constexpr char default_config[] = "mainnet\0" "default\0" "\0" "30\0" "10000\0" "720\0" "20\0";
	constexpr char mini_config[] = "mainnet\0" "mini\0" "\0" "30\0" "10000\0" "720\0" "20\0";
...
bool SideChain::get_shares(const PoolBlock* tip, std::vector<MinerShare>& shares, uint64_t* bottom_height, bool quiet) const
{
	...
	if (m_pool && !tip->m_parent.empty()) {
		const uint64_t h = n2pool::get_seed_height(tip->m_txinGenHeight);
...
bool SideChain::add_block(const PoolBlock& block)
{
	...
	if (new_block->m_verified) {
		if (!new_block->m_invalid) {
			...
			// Save it for faster syncing on the next n2pool start
...
void SideChain::print_status(bool obtain_sidechain_lock) const
{
	...
	// each dot corresponds to window_size / 30 shares, with current values, 720 / 30 = 24
...
bool SideChain::check_config() const
{
	...
	if ((m_chainWindowSize < 20) || (m_chainWindowSize > 2160)) {
		LOGERR(1, "pplns_window is invalid (must be between 20 and 2160)");
		return false;
	}
