...
static const char* seed_nodes[] = { "p2poolseeds.nevocoin.org", "" };
static const char* seed_nodes_mini[] = { "mini.p2poolseeds.nevocoin.org" "" };
...
P2PServer::P2PServer(n2pool* pool)
...
void P2PServer::update_peer_connections()
{
	...
	// Special case: when we can't find n2pool peers, scan through monerod peers (try 25 peers at a time)
	...
	if (!has_good_peers && ((m_timerCounter % 10) == 0)) {
		LOGERR(1, "no connections to other n2pool nodes, check your monerod/n2pool/network/firewall setup!!!");
		load_peer_list();
...
void P2PServer::check_host()
{
	...
	if (cur_time >= last_active + 300) {
		...
		const Params::Host& host = m_pool->current_host();
		LOGERR(1, "no ZMQ messages received from host " << host.m_displayName << " in the last " << dt << " seconds, check your monerod/n2pool/network/firewall setup!!!");
		m_pool->reconnect_to_host();
	}
}
...
void P2PServer::check_for_updates(bool forced) const
{
	...
	if ((forced || s.precalcFinished()) && m_newP2PoolVersionDetected && s.p2pool_update_available()) {
		...
		LOGINFO(0, log::LightCyan() << "* An updated P2Pool version is available, visit n2pool.io for more information *");
		LOGINFO(0, log::LightCyan() << "********************************************************************************");
	}
}
...
bool P2PServer::P2PClient::on_peer_list_request(const uint8_t*)
{
	...
	// - use first 8 bytes of the 16-byte raw IP address to send supported protocol version and n2pool version
	if (first) {
...
void P2PServer::P2PClient::handle_incoming_block(n2pool* pool, PoolBlock& block, std::vector<hash>& missing_blocks, bool& result)
{
	result = pool->side_chain().add_external_block(block, missing_blocks);
}

void P2PServer::P2PClient::post_handle_incoming_block(n2pool* pool, const PoolBlock& block, const uint32_t reset_counter, bool is_v6, const raw_ip& addr, std::vector<hash>& missing_blocks, const bool result)
{
