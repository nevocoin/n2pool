...
void p2pool_usage()
{
	printf("N2Pool %s\n"
...
#endif
		"--mini               Connect to n2pool-mini sidechain. Note that it will also change default p2p port from %d to %d\n"
		...
		"%s --host 127.0.0.1 --rpc-port 44231 --zmq-port 44233 --wallet YOUR_WALLET_ADDRESS --stratum 0.0.0.0:%d --p2p 0.0.0.0:%d\n\n",
		n2pool::VERSION,
		n2pool::log::MAX_GLOBAL_LOG_LEVEL,
		n2pool::DEFAULT_P2P_PORT,
		n2pool::DEFAULT_P2P_PORT_MINI,
#ifdef _WIN32
		"n2pool.exe"
#else
		"./n2pool"
#endif
		, n2pool::DEFAULT_STRATUM_PORT
		, n2pool::DEFAULT_P2P_PORT
	);
}
...
int p2pool_test()
{
	...
	n2pool::log::Stream s(buf);
	s << n2pool::log::hex_buf(hash, RANDOMX_HASH_SIZE) << '\0';
...
int main(int argc, char* argv[])
{
	...
	n2pool::init_crypto_cache();

	int result = static_cast<int>(curl_global_init_mem(CURL_GLOBAL_ALL, n2pool::malloc_hook, n2pool::free_hook, n2pool::realloc_hook, n2pool::strdup_hook, n2pool::calloc_hook));
	if (result != CURLE_OK) {
		return result;
	}

	try {
		n2pool::n2pool pool(argc, argv);
		result = pool.run();
	}
	catch (...) {
		result = 1;
