/*
 * This file is part of the Monero P2Pool <https://github.com/SChernykh/n2pool>
...
constexpr char FOUND_BLOCKS_FILE[] = "n2pool.blocks";

namespace n2pool {

n2pool::n2pool(int argc, char* argv[])
	: m_stopped(false)
	, m_updateSeed(true)
	, m_submitBlockData{}
	, m_zmqLastActive(0)
...
void n2pool::update_host_ping(const std::string& display_name, double ping)
{
	if (ping < 100) {
...
bool n2pool::calculate_hash(const void* data, size_t size, uint64_t height, const hash& seed, hash& result, bool force_light_mode)
{
	return m_hasher->calculate(data, size, height, seed, result, force_light_mode);
}

uint64_t n2pool::get_seed_height(uint64_t height)
{
...
bool n2pool::get_seed(uint64_t height, hash& seed) const
{
...
#ifdef WITH_RANDOMX
void n2pool::print_miner_status()
{
...
void n2pool::handle_tx(TxMempoolData& tx)
{
	if (!tx.weight || !tx.fee) {
...
void n2pool::handle_miner_data(MinerData& data)
{
#if TEST_MEMPOOL_PICKING_ALGORITHM
...
void n2pool::handle_chain_main(ChainMain& data, const char* extra)
{
	{
		WriteLock lock(m_mainchainLock);
	...
	if (!sidechain_id.empty()) {
		...
		if (block) {
			...
			const char* who = (block->m_minerWallet == w) ? "you" : "someone else in this n2pool";
...
void n2pool::submit_block_async(uint32_t template_id, uint32_t nonce, uint32_t extra_nonce)
{
	{
	...
	// If n2pool is stopped, m_submitBlockAsync is most likely already closed
	if (m_stopped) {
		LOGWARN(0, "n2pool is shutting down, but a block was found. Trying to submit it anyway!");
		submit_block();
		return;
	}
...
void n2pool::submit_block_async(std::vector<uint8_t>&& blob)
{
	{
		MutexLock lock(m_submitBlockDataLock);
...
bool init_signals(n2pool* pool, bool init);

void n2pool::on_stop(uv_async_t* async)
{
	n2pool* pool = reinterpret_cast<n2pool*>(async->data);
...
void n2pool::submit_block() const
{
...
bool n2pool::submit_sidechain_block(uint32_t template_id, uint32_t nonce, uint32_t extra_nonce)
{
...
void n2pool::update_block_template_async(bool is_alternative_block)
{
	// If n2pool is stopped, m_blockTemplateAsync is most likely already closed
	if (m_stopped) {
		return;
	}
...
void n2pool::update_block_template()
{
...
void n2pool::download_block_headers(uint64_t current_height)
{
...
bool n2pool::chainmain_get_by_hash(const hash& id, ChainMain& data) const
{
	ReadLock lock(m_mainchainLock);
...
bool n2pool::get_timestamps(uint64_t (&timestamps)[TIMESTAMP_WINDOW]) const
{
	ReadLock lock(m_mainchainLock);
...
void n2pool::update_median_timestamp()
{
...
void n2pool::parse_get_info_rpc(const char* data, size_t size)
{
	if (m_stopped) {
		return;
	}
	...
	if (info.busy_syncing || !info.synchronized) {
		LOGINFO(1, "nevocoind is " << (info.busy_syncing ? "busy syncing" : "not synchronized") << ", trying again in 1 second");
		std::this_thread::sleep_for(std::chrono::milliseconds(1000));
		get_info();
		return;
	}
...
void n2pool::parse_get_version_rpc(const char* data, size_t size)
{
	if (m_stopped) {
		return;
	}
...
void n2pool::get_miner_data(bool retry)
{
...
void n2pool::parse_get_miner_data_rpc(const char* data, size_t size)
{
	if (m_stopped) {
		return;
	}
...
bool n2pool::parse_block_header(const char* data, size_t size, ChainMain& c)
{
	rapidjson::Document doc;
	if (doc.Parse(data, size).HasParseError() || !doc.IsObject()) {
...
uint32_t n2pool::parse_block_headers_range(const char* data, size_t size)
{
	rapidjson::Document doc;
	if (doc.Parse(data, size).HasParseError() || !doc.IsObject()) {
...
void n2pool::api_update_network_stats()
{
	if (!m_api || m_stopped) {
		return;
	}
...
void n2pool::api_update_pool_stats()
{
	if (!m_api || m_stopped) {
		return;
	}
...
void n2pool::api_update_stats_mod()
{
	if (!m_api || m_stopped) {
		return;
	}
...
void n2pool::cleanup_mainchain_data(uint64_t height)
{
...
void n2pool::api_update_block_found(const ChainMain* data, const PoolBlock* block)
{
	if (!m_api || m_stopped) {
		return;
	}
...
bool n2pool::get_difficulty_at_height(uint64_t height, difficulty_type& diff)
{
	ReadLock lock(m_mainchainLock);
...
#ifdef WITH_RANDOMX
void n2pool::start_mining(uint32_t threads)
{
	stop_mining();
...
{
	n2pool* pool = reinterpret_cast<n2pool*>(handle->data);
...
bool n2pool::zmq_running() const
{
	ReadLock lock(m_ZMQReaderLock);
	return m_ZMQReader && m_ZMQReader->is_running();
}

const Params::Host& n2pool::switch_host()
{
	const std::vector<Params::Host>& v = m_params->m_hosts;
	return v[++m_currentHostIndex % v.size()];
}

void n2pool::reconnect_to_host()
{
	// If n2pool is stopped, m_reconnectToHostAsync is most likely already closed
	if (m_stopped) {
		return;
	}
...
{
	if (!m_params->valid()) {
		LOGERR(1, "Invalid or missing command line. Try \"n2pool --help\".");
		return 1;
	}
