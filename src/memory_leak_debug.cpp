...
void show_top_10_allocations()
{
	...
	{
		n2pool::MutexLock lock(allocation_lock);
...
NOINLINE void* operator new(size_t n) { return n2pool::allocate(n); }
NOINLINE void* operator new[](size_t n) { return n2pool::allocate(n); }
NOINLINE void* operator new(size_t n, const std::nothrow_t&) noexcept { return n2pool::malloc_hook(n); }
NOINLINE void* operator new[](size_t n, const std::nothrow_t&) noexcept { return n2pool::malloc_hook(n); }
NOINLINE void operator delete(void* p) noexcept { n2pool::free_hook(p); }
NOINLINE void operator delete[](void* p) noexcept { n2pool::free_hook(p); }
NOINLINE void operator delete(void* p, size_t) noexcept { n2pool::free_hook(p); }
NOINLINE void operator delete[](void* p, size_t) noexcept { n2pool::free_hook(p); }
